    ...
    <div class="content">
      ...
      <div class="item">
        ...
        <div class="item-content">
          <div class="list" v-for="(item, index) of addMemberListArr" :key="index">
            <ul>
              <li>{{item.name}}</li>
              <li>{{item.count}}</li>
            </ul>
      ...
      <div class="item item-center">
        ...
        <div class="item-content">
          <div class="list" v-for="(item, index) of activeMemberListArr" :key="index">
            <ul>
              <li>{{item.name}}</li>
              <li>{{item.count}}</li>
            </ul>
      ...
      <div class="item">
        ...
        <div class="item-content">
          <div class="list" v-for="(item, index) of tonsoilMemberListArr" :key="index">
            <ul>
              <li>{{item.name}}</li>
...
export default {
  ...
  data () {
    return {
      ...
      activeMemberListArr: [], // 活跃会员
      addMemberListArr: [], // 新增会员
      tonsoilMemberListArr: [] // 吨数会员
    }
  },
  created () {
    this.cityRankDetail()
    ...
    cityRankDetail () {
      ...
      this.$http.get('/webapi/bdview/gasolineDataBS/cityRankDetail').then(res => {
        if (res.result) {
          ...
          let tonsoilMemberList = res.data.tonsoilMemberList
          // 根据数组对象里面的某个属性值进行从大到小排序并截取前10个
          this.activeMemberListArr = activeMemberList.sort(this.compare('count', false)).slice(0, 10)
          this.addMemberListArr = addMemberList.sort(this.compare('count', false)).slice(0, 10)
          this.tonsoilMemberListArr = tonsoilMemberList.sort(this.compare('count', false)).slice(0, 10)
          // 是否启动轮询
...
<style scoped lang="stylus">
  ...
  .region-wrapper {
    ...
    flex 1
    margin-right vw(15)
    background url("https://img.gdoil.cn/cockpit/static/images/gasoline/qie_17.png") no-repeat
