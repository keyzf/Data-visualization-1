<template>
  <div class="diesel-beans-wrapper">
    <div class="item">
      <div id="dieselBeans3" class="diesel-beans"></div>
      <p class="text">产生柴油豆(万个)</p>
    </div>
    <div class="item">
      <div id="dieselBeans1" class="diesel-beans"></div>
      <p class="text">兑换非油品(万元)</p>
    </div>
    <div class="item">
      <div id="dieselBeans2" class="diesel-beans"></div>
  ...
  methods: {
    ...
    dieselBeanSituation () {
      this.$http.post('/webapi/bdview/thDataBS/dieselBeanSituation').then(res => {
        if (res.result === true) {
          let rs = res.data.today
          this.tdNoOil = this.$util.fixedFunc(rs.tdNoOil) || 0
          this.tdDieselBean = this.$util.fixedFunc(rs.tdDieselBean) || 0
    ...
    drawCharts1 () {
      ...
      let myChart = this.$echarts.init(document.getElementById('dieselBeans1'))
      // 绘制图表
      myChart.setOption({
        series: [
          {
    ...
    drawCharts2 () {
      // 基于准备好的dom，初始化echarts实例
      let myChart = this.$echarts.init(document.getElementById('dieselBeans2'))
      // 绘制图表
      myChart.setOption({
        series: [
          {
    ...
    drawCharts3 () {
      // 基于准备好的dom，初始化echarts实例
      let myChart = this.$echarts.init(document.getElementById('dieselBeans3'))
      // 绘制图表
      myChart.setOption({
        series: [
          {
...
<style scoped lang="stylus">
  ...
  .diesel-beans-wrapper {
    width: 100%
    height: 100%
    background: url("https://img.gdoil.cn/cockpit/static/images/trackHomeScreen/top_bg2.png?1") no-repeat
