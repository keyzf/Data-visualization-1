<template>
  <div class="wraper">
    <div class="wrap">
      <div id="washCarCharts" class="chart"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    basicCategoryDailyTurnover () {
      this.$http.get('/webapi/bdview/gasolineDataBS/carWashCountTrend').then(res => {
        if (res.result === true) {
    ...
    drawChart (nameLastYearData, countTheYearData, countLastYearData) {
      ...
      let myChart = this.$echarts.init(document.getElementById('washCarCharts'))
      // 绘制图表
      myChart.setOption({
...
<style scoped lang="stylus">
  ...
  .wraper {
    ...
    height: vh(240)
    color: #97a8f7
    background: url("https://img.gdoil.cn/cockpit/static/images/carMember/center_bg3_.png") no-repeat
