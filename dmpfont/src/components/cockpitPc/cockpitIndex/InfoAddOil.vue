<template>
  <div >
    <section class="member">
      <div class="member-title">最新加油客户信息</div>
      <section>
        <table>
          <thead class="thead">
            <tr>
          ...
          <tbody class="tbody" >
            <tr v-for="(item, index) in refuelList" :key="index">
...
export default {
  ...
  methods: {
    getNewRefuelUserInfo () {
      this.$http.post('/webapi/cockpit/getNewRefuelUserInfo', {
        page: 1,
        pageSize: 5
      }).then(res => {
        if (res.result === true) {
          this.refuelList = res.rows
        } else {
