<template>
  <div class="wraper">
    <div class="wrap">
      <div id="saleLinesCharts" class="chart"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    basicCategoryRetailDailyTurnover () {
      this.$http.post('/webapi/bdview/notOilDataBS/basicCategoryRetailDailyTurnover').then(res => {
        if (res.result === true) {
    ...
    drawChart (nameLastYearData, countTheYearData, countLastYearData) {
      ...
      let myChart = this.$echarts.init(document.getElementById('saleLinesCharts'))
      // 绘制图表
      myChart.setOption({
