<template>
  <div class="wraper">
    <div class="wrap">
      ...
      <div id="monthSaleChart" class="chart"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    theMonthBasicCategorySales () {
      this.$http.post('/webapi/bdview/notOilDataBS/theMonthBasicCategorySales').then(res => {
        if (res.result === true) {
    ...
    drawChartMonthSale (saleStyleData) {
      ...
      let myChart = this.$echarts.init(document.getElementById('monthSaleChart'))
      myChart.setOption({
        title: { // 顶部标题设置
          show: false,
          text: '月销售基础品类',
