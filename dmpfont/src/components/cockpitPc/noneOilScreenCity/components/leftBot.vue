<template>
  <div class="wraper">
    ...
    <div class="wrap">
      <div id="mainSaleChart1" class="chart"></div>
    </div>
    <div class="wraps">
      <div id="mainSaleCharts1" class="charts"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  //     if (this.cityCode !== '') {
  //       this.clearTimeout()
  //       console.log('leftBot' + this.cityCod)
  //       this.theMonthCoreCommoditySales()
  //     }
  //   }
  // },
  ...
  methods: {
    ...
    theMonthCoreCommoditySales () {
      this.$http.post('/webapi/bdview/municipalCompany/theMonthCoreCommoditySales', {cityCode: this.cityCode}).then(res => {
        if (res.result === true) {
    ...
    drawMonthChart (nameMonthData, countMonthData) {
      ...
      let myChart = this.$echarts.init(document.getElementById('mainSaleChart1'))
      // 绘制图表
      myChart.setOption({
        tooltip: {
          position: 'inside',
    ...
    drawYearChart (nameYearData, countYearData) {
      ...
      // 基于准备好的dom，初始化echarts实例
      let myChart = this.$echarts.init(document.getElementById('mainSaleCharts1'))
      // 绘制图表
      myChart.setOption({
        tooltip: {
          position: ['30%', '20%'],
