<template>
  <div class="wraper">
    <div class="wrap">
      <div id="saleLinesCharts1" class="chart"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  watch: {
    cityCode: {
      handler (newValue, oldValue) {
        ...
        this.basicCategoryRetailDailyTurnover()
      },
      deep: true
    }
  },
  ...
  methods: {
    // 基础品类零售日营业额
    basicCategoryRetailDailyTurnover () {
      this.$http.post('/webapi/bdview/municipalCompany/basicCategoryRetailDailyTurnover', {cityCode: this.cityCode}).then(res => {
        if (res.result === true) {
    ...
    drawChart (nameLastYearData, countTheYearData, countLastYearData) {
      ...
      var charts = {
        ...
        // value: [
        //   [551, 558, 533, 534, 555, 536, 517, 528, 559, 551, 531, 542, 553, 524, 565, 536, 527, 528, 529, 530, 551, 531, 552, 553, 524, 565, 536, 527, 528, 529],
        //   [760, 745, 780, 792, 730, 780, 792, 780, 750, 753, 752, 728, 725, 745, 765, 725, 768, 708, 753, 798, 780, 792, 780, 750, 753, 752, 728, 725, 755, 765]
        // ]
      }
      var color = ['rgba(183, 148, 17', 'rgba(81, 215, 248']
      var lineY = []
      ...
      let myChart = this.$echarts.init(document.getElementById('saleLinesCharts1'))
      // 绘制图表
      myChart.setOption({
        backgroundColor: '#000530',
        tooltip: {
          // position: ['45%', '20%'], // 提示框位置
          trigger: 'axis'
        },
        legend: {
          data: charts.names,
...
<style scoped lang="stylus">
  ...
  .wraper {
    ...
    height: 100%
    color: #97a8f7
    background: url("https://img.gdoil.cn/cockpit/static/images/noneOilScreen/bg_center_c.png") no-repeat
