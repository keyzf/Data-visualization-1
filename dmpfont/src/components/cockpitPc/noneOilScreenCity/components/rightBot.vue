<template>
  <div class="wraper">
    <div class="wrap">
      ...
      <div id="likeGoodsCharts1" class="chart"></div> -->
...
export default {
  ...
  methods: {
    ...
    customerMostLikeProductTop () {
      this.$http.post('/webapi/bdview/municipalCompany/customerMostLikeProductTop', {cityCode: this.cityCode}).then(res => {
        if (res.result === true) {
          let json = res.rows
          // 数据超出10条，取前10
          if (json.length > 10) {
    ...
    drawChart (dataList, yDataList, yDataList1, maxDataList) {
      ...
      let myChart = this.$echarts.init(document.getElementById('likeGoodsCharts1'))
      // 绘制图表
      myChart.setOption({
        title: {},
        tooltip: {
