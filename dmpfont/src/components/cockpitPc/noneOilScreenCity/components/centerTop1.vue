<template>
  <div class="wraper">
    <div class="wrap">
      <div id="saleLineCharts1" class="chart"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    basicCategoryDailyTurnover () {
      this.$http.post('/webapi/bdview/municipalCompany/basicCategoryDailyTurnover', {cityCode: this.cityCode}).then(res => {
        if (res.result === true) {
    ...
    drawChart (nameLastYearData, countTheYearData, countLastYearData) {
      ...
      let myChart = this.$echarts.init(document.getElementById('saleLineCharts1'))
      // 绘制图表
      myChart.setOption({
