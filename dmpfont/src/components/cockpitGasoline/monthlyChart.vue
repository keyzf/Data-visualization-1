    ...
    <div class="igs-flex igs-flex9 chart_list">
      <div class="chart_list_cont marbot1">
        ...
        <section class="chart">
          ...
          <div v-if="isHaveGoods" id="outlineChart" :style="{width: '100%', height: '330px'}"></div>
...
export default {
  ...
  methods: {
    ...
    getRedeemOnlineTopListByOilCode () {
      let that = this
      this.$http.post('/webapi/bdview/gasolineMobile/getRedeemOfflineTopListByOilCodeAndMonth', {
    ...
    drawChartOnline () {
      ...
      myChart.setOption({
        color: { // 图标渐变色设置
          ...
          y2: 1,
          colorStops: [{
            offset: 0, color: 'rgba(171, 153, 255, 1)' // 0% 处的颜色
          }, {
            offset: 1, color: 'rgba(118, 52, 255, 1)' // 100% 处的颜色
          }],
    ...
    paymentHabitsListByOilCode () {
      let that = this
      this.$http.post('/webapi/bdview/gasolineMobile/paymentHabitsListByOilCodeAndMonth', {
        oilCode: this.oilCode,
        month: this.curItemMonth
      }).then(res => {
        if (res.result === true) {
