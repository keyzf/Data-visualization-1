<template>
  <div class="cockpitIpad igs-flex"  id="content">
    ...
    <section class="container" style="flex:1;" id="offsetTop" >
...
export default {
  ...
  created () {
    window.document.title = '柴油移动端' // 设置页面标题
  ...
  methods: {
    ...
    stationListByUserId () {
      ...
      that.$http.get('/webapi/bdview/mobile/getUserOilStationList', {
        ...
        if (res.result === true) {
          var data = res.data
          // var data = [{account: '18660397153', id: 3356, phone: 18660397153, station: '中山安乐加油站中山安乐加油站中山安乐加油站中山安乐加油站中山安乐加油站中山安乐加油站-100039020054', username: '倪阳'}]
    ...
    choseTab (index, status) {
      ...
      this.status = status
      // 切换页面滚动条回到顶部
      document.getElementById('offsetTop').scrollTop = 0
    },
    // 选择油站
    selectoli (item) {
      console.log(item)
  ...
  components: {
    overallData: require('@/components/CockpitIpad/overallData.vue').default,
    todayData: require('@/components/CockpitIpad/todayData.vue').default,
    monthlyData: require('@/components/CockpitIpad/monthlyData.vue').default,
    clientManagement: require('@/components/CockpitIpad/clientManagement.vue').default
  }
}
</script>
...
.cockpitIpad{
  ...
  .tab_box{
      ...
      .selected_color{
        color:#4C4948;
        border-bottom: 3px solid #1696FF;
      }
      span {
