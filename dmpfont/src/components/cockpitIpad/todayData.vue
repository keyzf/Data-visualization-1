    ...
    <div class="box_top igs-flex igs-flex9 igs-flex10">
     ...
     <div class="data_box" v-if=" tadayList.length>0">
      <div class="box_b" v-for="(item, index) in tadayList" :key="index">
        ...
        <div class="box_b_b">
          <div class="igs-flex">
            <div class="box_b_l">柴油消费金额￥：<span>{{item.dieseConsumptionAmout|formatData0}}</span></div>
            <!-- <div class="box_b_c">消费次数：<span>3</span></div> -->
            <div class="box_b_l">柴油豆余额：<span>{{item.dieselBeanBalance|formatData0}}</span></div>
          </div>
           <div class="igs-flex">
            <div class="box_b_l">会员等级：<span>{{item.memberLeve|formatData}}</span></div>
            <div class="box_b_l">累计加油升数：<span>{{item.refuelTotal|formatData0}}</span></div>
          </div>
          <div class="igs-flex">
            ...
            <div class="box_b_l">兑换最多产品：<span class="name">{{item.redeemMostProduct|formatData}}</span></div>
          </div>
           <div class="igs-flex">
...
export default {
  ...
  methods: {
    ...
    todayStationInfoByOilCode () {
      ...
      that.$http.get('/webapi/bdview/thMobile/todayStationInfoByOilCode', {
    ...
    todayStationSalesRecordByOilCodeList () {
      let that = this
      this.$dialog.loading.open('加载中')
      that.$http.get('/webapi/bdview/thMobile/todayStationSalesRecordByOilCodeList', {
        oilCode: this.oilCode
      }).then(res => {
        this.$dialog.loading.close()
        if (res.result === true) {
    ...
    //   10. 获取线下兑换商品TOP5列表
    // GET /todyRedeemOfflineTopListByOilCode?oilCode=33250135&month=202002
    todyRedeemOfflineTopListByOilCode () {
      let that = this
      this.$dialog.loading.open('加载中')
      this.$http.post('/webapi/bdview/thMobile/todyRedeemOfflineTopListByOilCode', {
        oilCode: this.oilCode
      }).then(res => {
        this.$dialog.loading.close()
        if (res.result === true) {
    ...
    todyPaymentHabitsListByOilCode () {
      let that = this
      this.$dialog.loading.open('加载中')
      this.$http.post('/webapi/bdview/thMobile/todyPaymentHabitsListByOilCode', {
        oilCode: this.oilCode
      }).then(res => {
        this.$dialog.loading.close()
        if (res.result === true) {
    ...
    drawChartPayStyle () {
      // 图表颜色，从右上角开始
      var color = ['#A4DA73', '#BC89FE', '#FF7979', '#BC89FE', '#FFB45C', 'blue']
      var dataList = []
