    ...
    <div class="data_box">
      ...
      <div class="box_top igs-flex igs-flex9 igs-flex10">
        ...
        <div class="box_top_tab igs-flex igs-flex2">
          <div style="width:100%;text-align:center;">
            <p>产生柴油豆/万个</p>
            <p class="num">{{$util.fixedFunc(topStationData.produceDieselBeanTotal[0] / 10000) || 0}}</p>
        ...
        <div class="box_top_tab igs-flex igs-flex2">
          <div style="width:100%;text-align:center;">
            <p>兑换柴油豆/万个</p>
            <p class="num">{{$util.fixedFunc(topStationData.redeemDieselBeanTotal[0] / 10000) || 0}}</p>
        ...
        <div class="box_b"   v-for="(item, index) in monthDataList" :key="index">
          <div class="igs-flex igs-flex4 igs-flex2 phone_bg">
            <div class="box_b_t_l">手机号后四位：
              <span class="blue_color">{{item.phoneNum}}</span>
          ...
          <div class="box_b_b">
            <div class="igs-flex">
              <div class="box_b_l ellipsis">柴油消费金额￥：<span>{{item.dieseConsumptionAmout || 0}}</span></div>
              <div class="box_b_l is-show">消费次数：<span>{{item.consumption || 0}}</span></div>
              <div class="box_b_l ellipsis">柴油豆余额：<span>{{item.dieselBeanBalance || 0}}</span></div>
            </div>
            <div class="igs-flex">
              <div class="box_b_l igs-flex igs-flex2">油非互动券(张)：<span class="blue_color">{{item.couponCount}}</span> <img src="static/ipadImg/icon_go.png" alt=""></div>
...
export default {
  ...
  data () {
    return {
      ...
      monthDataList: ['', '', '', '', ''] // 消费记录列表
      // oilCode: '33250135' // 油站编码

    }
  },
  components: {
    MonthlyChart: require('@/components/cockpitIpad/monthlyChart.vue').default
  },
  props: {
    oilCode: String // 先屏蔽
  },
  ...
  methods: {
    ...
    stationInfoByOilCodeAndMonth () {
      ...
      this.$http.post('/webapi/bdview/thMobile/stationInfoByOilCodeAndMonth', {
    ...
    salesRecordListByOilCodeAndMonth () {
      let that = this
      this.$dialog.loading.open('加载中')
      this.$http.post('/webapi/bdview/thMobile/salesRecordListByOilCodeAndMonth', {
        oilCode: that.oilCode,
        month: that.curItemMonth
      }).then(res => {
        if (res.result === true) {
